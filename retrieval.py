...
def predict_population(start, end, suburbs):
            ...
            "data": [
                {
                    ...
                    "attribute": {"year": suburb[k + 1]},
                }
                for k in range(len(years))
            ],
            "value_attribute": "year",
            "time_points": predicted_years,
        }
    ...
    for i in range(len(data)):
        suburb = data[i][0]
        new_suburb_data = [suburb]
        for year in range(start, end + 1):
            new_suburb_data.append(data_dict[suburb][str(year)])
        ret_data.append(new_suburb_data)
    return ret_data
...
def populationAll(startYear, endYear):
    ...
    for i in range(0, len(res_suburbs)):
        res_suburbs[i] = [res_suburbs[i][0]] + res_suburbs[i][
            startYear - 2020 : endYear - 2019
        ]
